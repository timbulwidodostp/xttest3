...
* mod 1.0.8 to deal with T=2 variance becoming nonzero
...
// 1.0.8 : guard against a near-zero value caused by T-2
	qui replace `v' = . if `v'<1e-12
	qui summ `e'
...
* W' is sum over units
	...
	dis in gr "chi2 (`ng')  =" in ye _col(16) %12.2f `waldval'  
	dis in gr "Prob > chi2 = " in ye _col(22) %8.4f chiprob(`ng',`waldval') _n
